...
OMR::BytecodeBuilder::Switch(const char *selectionVar,
   ...
   if (*defaultBuilder == NULL)
      *defaultBuilder = _methodBuilder->OrphanBytecodeBuilder(_bcIndex, _name);
   ...
   TR::BytecodeBuilder *breakBuilder = _methodBuilder->OrphanBytecodeBuilder(_bcIndex, _name);
   ...
   for (int32_t c=0;c < numCases;c++)
      ...
      if (!caseFallsThrough[c])
         {
         handler = _methodBuilder->OrphanBytecodeBuilder(_bcIndex, _name);
         //TODO hack!!!
         handler->propagateVMState(vmState());

         if (caseBuilders[c] == NULL)
            caseBuilders[c] = _methodBuilder->OrphanBytecodeBuilder(_bcIndex, _name);
         handler->AppendBuilder(caseBuilders[c]);
         caseBuilders[c]->propagateVMState(handler->vmState());

         // handle "break" with a separate builder so user can add whatever they want into caseBuilders[c]
         TR::BytecodeBuilder *branchToBreak = _methodBuilder->OrphanBytecodeBuilder(_bcIndex, _name);
         //TODO hack
         branchToBreak->propagateVMState(handler->vmState());
